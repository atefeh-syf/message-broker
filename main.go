...
func main() {
	...
	subscriber := broker.Subscribe("topic1")
	go func() {
		for {
			select {
	...
	broker.Publish("topic1", "This is a test message.")

	time.Sleep(2 * time.Second)
	broker.Unsubscribe("topic1", subscriber)

	broker.Publish("topic1", "This message won't be received.")
